...
- `top.v`  
  Top-level module integrating SPI, accelerometer control, clock generation, and display logic.
  ```verilog
    `timescale 1ns / 1ps
  module top(
...
- `spi_master.v`  
  Implements SPI master FSM handling CS, SCLK, MOSI, and MISO.
  ```verilog
  `timescale 1ns / 1ps
...
- `iclk_gen.v`  
  Generates SPI-compatible clock from the system clock.
...
<img width="40%" alt="image" src="https://github.com/user-attachments/assets/1f772682-00b9-4969-a4b7-9ecff3d5c940" />
